...
fit_no_distigfs<- gadget.fit( gd = '~/Documents/Hafro/fishvice/19-gss/exploratory_models/gadget/01-new_ass_no_distigfs_mt_s6')
...
fit_no_igfs<- gadget.fit( gd = '~/Documents/Hafro/fishvice/19-gss/exploratory_models/gadget/01-new_ass_no_igfs_mt_s6')
...
both <- bind.gadget.fit(fit_no_distigfs, fit_no_igfs)
...
si_plot <-
  ...
  scale_color_discrete(name = "model", labels = c("with IGFS SI", "no IGFS"))
print(si_plot)
```
...
tmp_no_distigfs_bub <- plot(fit_no_distigfs,data='catchdist.fleets', type = 'bubble')

gridExtra::grid.arrange(
  tmp_no_distigfs_bub$aldist+ 
    geom_text(aes(x = x, y = y, label = label), data = tibble(x = 2011, y= 25, name = c('aldist.aut', 'aldist.comm'), label = 'with IGFS SI'), size = 5, color = 'black') +
    xlab('Year')+
    ylab('Age'),  
...
gridExtra::grid.arrange(
  tmp_no_distigfs_bub$ldist+ 
    geom_text(aes(x = x, y = y, label = label), data = tibble(x = 2011, y= 60, name = c('ldist.aut', 'ldist.comm'), label = 'with IGFS SI'), size = 5, color = 'black')+
    xlab('Year')+
    ylab('Length (cm)'),  
  tmp_no_igfs_bub$ldist+ 
    geom_text(aes(x = x, y = y, label = label), data = tibble(x = 2011, y= 60, name = c('ldist.aut', 'ldist.comm'), label = 'no IGFS'), size = 5, color = 'black')+
    xlab('Year')+
    ylab('Length (cm)')  
)
```
...
suitab_plot <-
  ...
  geom_line() +
  scale_linetype_discrete(name = "model", labels = c("with IGFS SI", "no IGFS"))+ 
  xlab('Length (cm)') +
...
tmp <- fit_no_distigfs$catchdist.fleets %>% dplyr::ungroup() %>% 
  tidyr::nest(-name) %>% 
  dplyr::mutate(plots = purrr::map(data, 
                                   function(x) {
                                     if (length(unique(x$age)) > 1) 
...
tmp_no_distigfs_gro <- tmp$plots %>% purrr::set_names(., tmp$name)
  ...
  tmp_no_distigfs_gro$aldist.aut+ 
    geom_text(aes(x = x, y = y, label = label), data = tibble(year = 2000, step = 4, x = 15, y= 15, label = 'with IGFS SI'), size = 5, color = 'red'),  
...
#  all$stockdist %>%
  ...
  mutate(model2 = model,
        model = ifelse(model2 %in% c(1,3), 'with IGFS SI', 
...
tot_bio_plot <-
  ...
  scale_linetype_discrete(name = "model", labels = c("with IGFS SI", "no IGFS")) + theme_bw()
print(tot_bio_plot)
```
...
SR_plot <-
  ...
  mutate(model = ifelse(model==1, 'with IGFS SI', 
                        ifelse(model==2, 'no IGFS', 'with IGFS SI, time-variable maturity'))) %>% 
  arrange(year) %>% 
  ggplot() + 

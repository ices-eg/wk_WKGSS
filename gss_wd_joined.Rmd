...
The results from the long-term simulations are shown in Figure 30.  $F_{lim}$, resulting in 50% long-term probability of SSB $>B_{lim}$, was estimated as `r round(f.lim,2)`. As the CV of $F$ in 2019 is 0.25, $F_{pa}$ is estimated as `r round(f.lim,2)`$/1.508$ = `r round(f.pa,3)`. 
...
assumptions, is roughly 9 thousand tonnes with a 90% confidence interval of for the yield ranging 0 to almost 17 thousand tons. $F_{msy}$ was estimated to be `r round(f.msy,3)`, corresponding with $H_{msy}$ of `r round(hr_msy,3)`. 
...
f_progn.plot <- 
  ...
  geom_hline(yintercept = f.05, color = 'deepskyblue') +
...
-   | $F_{msy}$ | `r round(f.msy, 3)`| Median $F$ that maximises the median long-term catch in stochastic simulations with 7-year block-bootstrapped recruitment, scaled according to a hockey stick recruitment function with the breakpoint set to $B_{lim}$.
-   | $F_{p.05}$ | `r round(f.05,3)`| The fishing mortality that has an annual 5% probability of  of SSB < $B_{lim}$.
------|-------|------|----------------
...
-   | $B_{pa}$  |`r round(bpa,3)` kt | $B_{lim}*e^{1.645*\sigma}$ where $\sigma=0.2$
-   | $F_{lim}$ |`r round(f.lim,2)`| $F$ corresponding to 50% long-term probability of SSB > $B_{lim}$ 
-    | $F_{pa}$ |`r round(f.pa,3)`| $F_{lim}/e^{1.645*\sigma}$ where $\sigma=0.25$
-------|------|----|----------------
MSY advice rule | $F_{msy}$ |  `r round(ftarget,3)`| $F$ such that $F\leq F_{msy}$, $F\leq F_{pa}$, and $F\leq F_{0.05}$, long-term yield is consistent with MSY while leading to high stock biomass 
...
## Application of reference points in the advice rule
  ...
  $$
  while $B_{lim} \leqslant SSB_y < B_{trigger}$, and
  
  $$
  TAC_y = 0
  $$
  while $SSB_y \leqslant B_{lim}$.
...
However, as a result of this uncertainty, precautionary reference points control the proposed ICES MSY advice rule: $F_{p05}$ controls $F_{msy}$ and MSY $B_{trigger}$ was set using $B_{pa}$. Therefore the proposed MSY advice rule takes into account this large amount of uncertainty, and is deemed precautionary and an improvement from the category 3 assessment currently used. Should methods be found to reduce the uncertainty in the model during the next benchmark (e.g., methods for smoothing input survey index data, etc.), and biomass and recruitment appear to continue in an upward trends, then reference point definitions could be revisited.

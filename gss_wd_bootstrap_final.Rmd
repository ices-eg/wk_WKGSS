...
for $B_{lim}$ can be calculated based on the inverse of the standard factor, $e^{\sigma * 1.645}$ where $\sigma$ is 0.2 (which is close to the bootstrap-estimated CV of SSB in the last year, 0.25), used for calculating $B_{pa}$ from $B_{lim}$. Therefore, a proxy for $B_{lim}$ could be set at $B_{pa}/e^{1.645*0.2} = 17.38/1.39 = 12.50 kt$. 
...
ranging from 0 to 0.6. For each run of the management strategy evaluation, 1 of the 100 bootstrap runs was forward projected with 1 of 10 randomly generated bootstrapped recruitment, yielding 1000 stochastic simulations. A series of F_{target} values (0 - 0.6 in steps of 0.01) were then tested by projecting harvesting of each F_{target} for 100 years under in each of the 1000 simulations (61 000 in total).  
...
$F_{msy}$ was estimated to be 0.24. The
...
```{r exampleFs, echo=FALSE, fig.width = 10, fig.height= 6, fig.cap = "Greater silver smelt in 5a and 14. Projected catches, spawning stock biomass and fishing mortality for select target harvest rates. Red and black dashed horizontal lines represent the limit and pa reference points respectively for SSB and F. "}
...
When considering the candidate harvest rate for Greater silver smelt in 5a and 14 one may want to investigate the marginal gain of increasing the harvest rate in terms of equilibrium yields. Even with catch rates as low as 0.13 the equilibrium yields are roughly comparable with that of the $F_{msy}$.  
...
while $SSB_y > B_{pa}$,  
$$
TAC_y = C_{asyt}*\frac{SSB_y}{B_{pa}}
$$
while $B_{pa} \leqslant SSB_y < B_{lim}$, and 
$$
TAC_y = 0
$$

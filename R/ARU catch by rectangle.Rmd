...
# load(file.path(datapath, "rdata/eez.df.RData"))
# load(file.path(datapath, "rdata/depth200.df.RData"))
...
files.list <- list.files(path=file.path(datapath, "combined"), pattern="arg.xlsx", full.names=TRUE )
...
for (f in files.list) {
  ...
  # add to dataset
  catch_by_species_year_country_raw <- bind_rows(catch_by_species_year_country_raw, t)
}

...
#- Set path
# dataPath  <- "C:/Downloads/user274-ARU.27.5b6a_WKGSS2020_FINAL/ARU.27.5b6a_WKGSS2020_FINAL/data/"
dataPath  <- "D:/iWKGSS 2020/06. Data/aru.27.5b6a/Model/SAM/GLL2019_FBS_FINAL1/data/"

#- Load data
setwd(dataPath)
# cn        <- stockassessment::read.ices("cn.dat")
cn        <- stockassessment::read.ices("D:/TEMP/cn.dat")

# cw        <- stockassessment::read.ices("cw.dat")
cw        <- stockassessment::read.ices("D:/TEMP/cw.dat")
...
# ARG@catch.n@.Data[ARG@catch.n<1] <- 0

ARG@catch.wt[]      <- t(cw)
ARG@catch.wt@.Data[ARG@catch.wt<0] <- 0
...
ARG@mat[]           <- t(mo[-nrow(mo),])
ARG@m[]             <- t(nm[-nrow(nm),])
ARG@harvest.spwn[]  <- t(pf[-nrow(pf),])
ARG@m.spwn[]        <- t(pm[-nrow(pm),])
ARG@stock.wt[]      <- t(sw[-nrow(sw),])
ARG@name            <- "GSS5b6a"
ARG@desc            <- "Greater Silversmelt 5b-6a"

# plot of catch at age consistency
...
ARG@catch.n[ac(pg),ac(2005:2006)]   <- colSums(ARGnopg@catch.n[ac(pg:dims(ARGnopg)$max),ac(2005:2006)],na.rm=T)
ARG@catch.wt[ac(pg),ac(2005:2006)]  <- colSums(ARGnopg@catch.n[ ac(pg:dims(ARGnopg)$max),ac(2005:2006)] *
                                               ARGnopg@catch.wt[ac(pg:dims(ARGnopg)$max),ac(2005:2006)],na.rm=T) /
                                       colSums(ARGnopg@catch.n[ ac(pg:dims(ARGnopg)$max),ac(2005:2006)],na.rm=T)
...
save(ARG, ARG.tun, ARG.ctrl, file="D:/TEMP/arg.RData")


#- Run initial SAM to define states
ARG.sam             <- FLSAM(ARG,ARG.tun,ARG.ctrl)
defAIC              <- AIC(ARG.sam); defAIC
xyplot(value + lbnd + ubnd ~ age | year,data=f(ARG.sam),type="b",col=c(1,"grey","grey"),lty=c(2,1,1),pch=19,scale=list(y="free"))

#---------------------
#- Add correlation structure for surveys
#---------------------

ARG.ctrl@cor.obs[2,3:9]           <- c(103:108,108)
ARG.ctrl@cor.obs[3,1:2]           <- c(201:202)
...
xyplot(value + lbnd + ubnd ~ age | fleet,data=cor.obs(ARG.samcorObs),type="l",col=c(1,"grey","grey"),lty=c(2,1,1),ylim=c(0,5))
#- Decision
ARG.ctrl@cor.obs[2,3:9]           <- c(rep(101,3),rep(102,2),rep(103,2))
ARG.ctrl@cor.obs[3,1:5]           <- 201
ARG.ctrl                          <- update(ARG.ctrl)
ARG.samcorObs                     <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.sam)
corObsAIC                         <- AIC(ARG.samcorObs); corObsAIC
...
ARG.ctrl@catchabilities["SummerSurvey",ac(4:6)] <- c(1,2,3) + 101
ARG.ctrl                                        <- update(ARG.ctrl)
...
#- Decision
ARG.ctrl@catchabilities["PairTrawl",ac(6:13)]   <- c(1,2,3,4,5,5,5,5)
ARG.ctrl@catchabilities["SummerSurvey",ac(4:6)] <- c(1,2,3) + 101
ARG.ctrl                                        <- update(ARG.ctrl)
ARG.samcatch                                    <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.samcatch)
catchAIC                                        <- AIC(ARG.samcatch); catchAIC

#---------------------
#- Configure Observation variances
#---------------------
ARG.ctrl@obs.vars["catch unique",]            <- c(4:16,16)
ARG.ctrl@obs.vars["PairTrawl",ac(6:13)]       <- c(106:112,112)
ARG.ctrl@obs.vars["SummerSurvey",ac(4:6)]     <- c(204:205,205)
ARG.ctrl                                      <- update(ARG.ctrl)
ARG.samobsVar                                 <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.samcatch)
obsVarAIC                                     <- AIC(ARG.samobsVar); obsVarAIC
xyplot(value ~ age | fleet,data=obs.var(ARG.samobsVar),type="b",pch=19,col=1)
#- Decision
ARG.ctrl@obs.vars["catch unique",]            <- c(1,rep(5,13))
ARG.ctrl@obs.vars["PairTrawl",ac(6:13)]       <- c(106,106,rep(107,4),rep(108,2))
ARG.ctrl@obs.vars["SummerSurvey",ac(4:6)]     <- c(rep(204,3))
ARG.ctrl                                      <- update(ARG.ctrl)
ARG.samobsVar                                 <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.samcatch)
obsVarAIC                                     <- AIC(ARG.samobsVar); obsVarAIC
...
ARG.ctrl@f.vars["catch unique",]  <- c(4:16,16)
ARG.ctrl                          <- update(ARG.ctrl)
ARG.samfvar                       <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.samcorObs)
...
#- Decision
ARG.ctrl@f.vars["catch unique",]  <- c(1,1,rep(2,10),3,3)
ARG.ctrl                          <- update(ARG.ctrl)
...
ARG.retrocorF2                    <- retro(ARG,ARG.tun,ARG.ctrl,retro=5)
ARG.ctrl@cor.F                    <- 1
ARG.samcorF1                      <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.samfvar)
ARG.retrocorF1                    <- retro(ARG,ARG.tun,ARG.ctrl,retro=5)
ARG.ctrl@cor.F                    <- 0
ARG.samcorF0                      <- FLSAM(ARG,ARG.tun,ARG.ctrl,starting.values=ARG.samfvar)
ARG.retrocorF0                    <- retro(ARG,ARG.tun,ARG.ctrl,retro=5)
...
storeMohnsRho["ssb","corF2"]      <- mean(mohns.rho(ARG.retrocorF2,type="ssb",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["fbar","corF2"]     <- mean(mohns.rho(ARG.retrocorF2,type="fbar",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["rec","corF2"]      <- mean(mohns.rho(ARG.retrocorF2,type="rec",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["ssb","corF1"]      <- mean(mohns.rho(ARG.retrocorF1,type="ssb",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["fbar","corF1"]     <- mean(mohns.rho(ARG.retrocorF1,type="fbar",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["rec","corF1"]      <- mean(mohns.rho(ARG.retrocorF1,type="rec",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["ssb","corF0"]      <- mean(mohns.rho(ARG.retrocorF0,type="ssb",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["fbar","corF0"]     <- mean(mohns.rho(ARG.retrocorF0,type="fbar",ref.year=2019,span=5)[1:5,1])
storeMohnsRho["rec","corF0"]      <- mean(mohns.rho(ARG.retrocorF0,type="rec",ref.year=2019,span=5)[1:5,1])
storeMohnsRho
#- Decision
ARG.ctrl@cor.F                    <- 2
...
ARG.sam                           <- FLSAM(ARG,ARG.tun,ARG.ctrl)
ARG.ctrl@residuals                <- FALSE
ARG.retro                         <- retro(ARG,ARG.tun,ARG.ctrl,retro=5)
...
pdf(file.path(path,"plots_diagnostics.pdf",sep=""))

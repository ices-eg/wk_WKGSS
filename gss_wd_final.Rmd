...
SR_plot <-
  ...
  left_join(fit_no_igfs$res.by.year %>% 
              ungroup() %>% 
              filter(stock=='gssimm') %>%
              select(year, recruitment) %>% 
              mutate(`Recruitment in millions (age 5)` = recruitment/1000000)) %>% 
  arrange(year) %>% 
  ggplot() + 
  #geom_point(aes(x = `Spawning stock biomass (1000s tonnes)`, y = `Recruitment (millions)`, color = model)) +
  geom_text(aes(x = `Spawning stock biomass (1000s tonnes)`, y = `Recruitment in millions (age 5)`, label = year)) +
  geom_path(aes(x = `Spawning stock biomass (1000s tonnes)`, y = `Recruitment in millions (age 5)`), alpha = 0.3) +
  geom_vline(aes(xintercept = minssb), lty = 2, data = fit_no_igfs$res.by.year %>% 
               ungroup() %>% 
               filter(stock=='gssmat') %>%
               select(year, total.biomass) %>% 
               filter(year > 1999) %>% 
